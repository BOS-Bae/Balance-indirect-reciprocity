...
int main(int argc, char **argv){
	...
	avg_E = avg_c = 0.0;
	T = 0.0;
	double avg_1_t, avg_2_t, avg_3_t;
	avg_1_t = avg_2_t = avg_3_t = 0.0;
	...
	for (b=0;b<MCS;b++){
		if (b >= t_thm - 1){
			if ((b-t_thm) % t_ms == 0) {
				...
				data_sl[t_idx][1] = avg_2_t;
				data_sl[t_idx][2] = (avg_3_t - avg_2_t*avg_2_t)/(data[b][0]*data[b][0]);

				t_idx += 1;
				avg_2_t = avg_3_t = 0.0;
			}
			avg_2_t += data[b][1]/t_ms;
			avg_3_t += data[b][2]/t_ms; // E squared
		}	
	}
	...
	for (b=0;b<n_sample;b++){
		...
		//avg_c += ((data[b][2] - data[b][1]*data[b][1])/(L*L*data[b][0]*data[b][0]))/n_sample;
	}
	...
	for (b=0;b<num_sample;b++){
		...
		//std_err_c += pow((((data[b][2] - data[b][1]*data[b][1])/(L*L*data[b][0]*data[b][0]) - avg_c)),2) / (n_sample-1);
	}
