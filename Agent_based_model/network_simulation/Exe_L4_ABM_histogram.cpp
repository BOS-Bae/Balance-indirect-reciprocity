...
void ABM_complete(int rule_num, vector<vector<int>> &mat_i, vector<double> &f_s, int N){
	random_device rd;
	mt19937 gen(rd());
	...
	while (true){
		...
		if (t > 1) {
    	    ...
    	    switch (rule_num) {
    	      case 4 :
    	        update_od.push_back(L4_rule(mat_i, o, d, r, 0, 0));
    	        break;
    	      case 6 :
    	        update_od.push_back(L6_rule(mat_i, o, d, r, 0, 0));
    	        break;
					}
			}
		}
    	    ...
    	    switch (rule_num) {
    	      case 4 :
    	        update_od.push_back(L4_rule(mat_i, o, d, r, 1, 0));
    	        break;
    	      case 6 :
    	        update_od.push_back(L6_rule(mat_i, o, d, r, 1, 0));
    	        break;
					}
			}
		}
...
int main(int argc, char *argv[]) {
	if (argc < 3) {
		cout << "./frag_prob N rule_num n_sample \n";
		exit(1);
	}
	int N = atoi(argv[1]);
	...
	vector<double> f_result_avg(n_bal, 0);
		
	int init_confi_idx;
	for (int ns = 0; ns < n_sample; ns++) {
		vector<vector<int>> mat_i;
		vector<vector<int>> subsets_i;
		vector<int> cluster1; vector<int> cluster2;
		for (int i = 0;	i < N; i++) {
			if (i < (int)((double)N/2.0) ) cluster1.push_back(i); 
			else cluster2.push_back(i); 
		}
		subsets_i.push_back(cluster1);
		subsets_i.push_back(cluster2);
		subset_to_mat(subsets_i, mat_i, N);

		vector<vector<int>>	exchanged_i;
		exchange_vectors(subsets_i, exchanged_i);

		int org_idx_i = subset_to_idx(subsets_i, N);
		int alter_idx_i = subset_to_idx(exchanged_i, N);
		if (org_idx_i < n_bal) init_confi_idx = org_idx_i;	
		else init_confi_idx = alter_idx_i;
	
		vector<double> f_s(n_bal_tot, 0);
		vector<double> f_result(n_bal, 0);
		
		ABM_complete(rule_num, mat_i, f_s, N);
		...
		for (int i=0; i<n_bal; i++) f_result_avg[i] += f_result[i];
		//for (int i=0; i<n_bal_tot; i++) {
		//	cout << f_s[i] << " ";
		//	summ += f_s[i];
		//}
		...
		double summ = 0;
		cout << N << " "<< init_confi_idx << " ";
		for (int i=0; i<n_bal; i++) {
			if (f_result_avg[i] != 0) cout << i << " " << f_result_avg[i]/(double)n_sample << " ";
			summ += f_result_avg[i]/(double)n_sample;
		}
		cout << summ << "\n";
	return 0;
