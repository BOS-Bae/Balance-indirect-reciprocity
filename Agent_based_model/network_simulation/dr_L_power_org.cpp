...
int main(int argc, char *argv[]) {
  ...
  if ((argc < 6) || (atoi(argv[3]) != 4 && atoi(argv[3]) != 6 && atoi(argv[3]) != 7)
      || (atoi(argv[4]) > 2)) {
    printf("./dr_L_power_org err iter rule_num init_vect_idx, bal_idx flip_idx \n");
...
int L4_rule(int mat_f[][N], int o, int d, int r, int idx_err) {
  ...
  int val = mat_f[o][d];
	int act_dr = idx_err == 0 ? mat_f[d][r] : -mat_f[d][r];
...
void power_method(double err, int iter, int rule_num, int init_vect_idx, int bal_idx, int flip_idx) {
  ...
  */
  ...
  if (init_vect_idx == 0) {
    ...
    char result[100];
    sprintf(result, "./dr_N%dL%d_e%d_bal%d.dat", N, rule_num,(int) log10(err), bal_idx);
    opening.open(result);
    ...
    char result[100];
    sprintf(result, "./dr_N%dL%d_flip%d.dat", N, rule_num, flip_idx);
    opening.open(result);
        ...
        for (int y = 0; y < N; y++) {
          for (int idx_n = 0; idx_n < 2; idx_n++) {
          	...
          	for (int l = 0; l < N; l++) {
          		switch (rule_num) {
          		  case 4 :
          		    updated_od.push_back(L4_rule(mat_f, l, x, y, idx_n));
          		    break;
          		  case 6 :
